...
$(function () {
  ...
  email_app_list.find('.email-user-list li').on('click', function (e) {

    var email_item = this; 
    
    fetch( '/correos/' + email_item.dataset.id )
      .then( response => response.json())
      .then( data => {
          if ( data.status ) {
            
            console.log(data); 
            email_app_details.find(".media-body .personal_name").text( data?.contacto?.nombres ?? 'Nombre') ;
            email_app_details.find(".media-body .email").text('<' + data.correo.correo_desde  + '>' );

            email_app_details.find(".email-detail-title").text(data.correo.asunto );
            email_app_details.find(".body-email").html(data.correo.texto );
            var template = document.getElementById('template-attchement').content;
            if ( data.adjuntos ){
              data.adjuntos.forEach( ( archivo ) => {

                 var clone =  template.cloneNode(true);

                 clone.querySelector('.attchement-text').textContent = archivo.filename;
                 clone.querySelector('.attchement-url').setAttribute( "href", clone.querySelector('.attchement-url').getAttribute("href") + archivo.archivo );
                 email_app_details.find(".file-list").append( clone );
