...
use App\Facades\DB;
use App\Facades\PaginationQuery;
    ...
    if($this->action == 'distinct') {
      if(!empty($this->inputs['column'])) {
        if(($ee = $this->eventColumn('edit', $this->inputs['column'])) || true) {
          ...
          $valores = array_combine(array_map(function($n) { return $n->value; }, $valores), $valores);
...
//    $query = $this->queryAddFilters($query);
    ...
    if(strpos($query, '--filters') !== FALSE  || true) {
      ...
      if(!empty($this->filters)) {
        foreach($this->filters as $k => $f) {
          ...
          if(in_array('nu11', $f)) {
            $f = array_filter($f, function($n) { return $n !== 'nu11'; });
            if(!empty($f)) {
...
//    }
  ...
  public function on($event, $field, $cb) {
    if(is_array($field)) {
      foreach($field as $f) {
