...
<div>
    <div class="card shadow-none quill-wrapper p-0">
      ...
      <form action="{{ route('correos.store') }}" method="POST" id="compose-form" enctype="multipart/form-data">
      @csrf
...
@foreach(App\User::perfiles(null) as $r)
                <option value="{{ $r->id }}">{{ $r->cargo }} ({{ $r->correo }})</option>
@endforeach
             </select>
            </div>
            <div class="form-label-group">
              <input type="email" name="para" id="emailTo" class="form-control" placeholder="Para" required>
            ...
            <div class="form-label-group">
              <input type="text" name="asunto" id="emailSubject" class="form-control" placeholder="Asunto">
        ...
        <div class="card-footer d-flex justify-content-end pt-0">
          ...
          <button type="submit" class="btn-send btn btn-primary">
            <i class='bx bx-send mr-25'></i> <span class="d-sm-inline d-none">Enviar</span>
          </button>
        </div>
      </form>
      <!-- form start end-->
