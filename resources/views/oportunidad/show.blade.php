...
@elseif(empty($oportunidad->aprobado_el))
  <a href="/oportunidades/{{ $oportunidad->id }}/aprobar" class="btn btn-sm btn-success mr-25">Aprobar</a>
  <a data-confirm-input="¿Por qué desea Rechazarlo?" href="/oportunidades/{{ $oportunidad->id }}/rechazar" class="btn btn-sm btn-danger mr-25">Rechazar</a>
@elseif(empty($oportunidad->revisado_el))
  <a href="/oportunidades/{{ $oportunidad->id }}/revisar" class="btn btn-sm btn-success mr-25">Revisar</a>
  <a data-confirm-input="¿Por qué desea Rechazarlo?" href="/oportunidades/{{ $oportunidad->id }}/rechazar" class="btn btn-sm btn-danger mr-25">Rechazar</a>
@elseif($oportunidad->estado == 'PENDIENTE')
  <a data-confirm-input="¿Por qué desea Rechazarlo?" href="/oportunidades/{{ $oportunidad->id }}/rechazar" class="btn btn-sm btn-danger mr-25">Rechazar</a>
@else
  <a data-confirm-input="¿Por qué desea Archivarlo?" href="/oportunidades/{{ $oportunidad->id }}/archivar" class="btn btn-sm btn-danger mr-25">Archivar</a>
@endif
    </div>
  </div>
  <div class="row">
...
@if(!empty($oportunidad->licitacion_id))
  ...
  <div class="col-6">
    <div class="card">
      <div class="card-content">
        <div class="card-body">
        @include('licitacion.cronograma', ['licitacion' => $oportunidad->licitacion()])
        </div>
      </div>
    </div>
  </div>
            ...
            @if(empty($oportunidad->revisado_el) && false)
             ...
             @elseif(empty($e->cotizacion) && strtotime($oportunidad->fecha_participacion_hasta) > time())
              <div class="text-center">
                <a class="btn btn-sm btn-success" href="/oportunidades/{{ $oportunidad->id }}/interes/{{ $e->id }}">Interés</a>
             </div>
            @elseif(empty($e->cotizacion) && strtotime($oportunidad->fecha_participacion_hasta) <= time())
                  ...
                  @endif
                   @if(!$e->cotizacion->estado()['timeout'])
                    @if(empty($e->cotizacion->participacion_el))
                      <a href="/cotizaciones/{{ $e->cotizacion->id }}/registrarParticipacion" class="btn btn-sm btn-info mr-25">Registrar Participación</a>
                    @endif
                  @endif
                </li>
                <li class="StepProgress-item is-done">
                  ...
                  @endif
                   @if(!$e->cotizacion->estado()['timeout'])
                    @if(empty($e->cotizacion->propuesta_el))
                      <a href="/cotizaciones/{{ $e->cotizacion->id }}/registrarPropuesta" class="btn btn-sm btn-dark">Enviar Propuesta</a>
                    @endif
                  @endif
                </li>
                </ul>
