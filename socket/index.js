...
function broadcast_tenant(tenant_id, evento, data) {
  var rp = [];
  for (var tid in ls) {
          ...
          if(ls[tid].hasOwnProperty(uid)) {
            for (var did in ls[tid][uid].devices) {
              if(ls[tid][uid].devices.hasOwnProperty(did)) {
                console.log('ENVIAR', did, evento, data);
                ls[tid][uid].devices[did].client.emit(evento, data);
              }
            }
          }
        }
...
app.post('/api/tenant/call', (req, res) => {
  console.log('DATA-TENANT', req.body);
  broadcast_tenant(req.body.tenant_id, 'caller', req.body);
...
io.on('connection', function(client) {
  ...
  client.on('state', function(data) {
    ...
    ls[device.tenant_id][device.user_id].states[data.key] = data.value;
  });
  client.on('todo_add', function(data) {
    ...
    broadcast_tenant(device.tenant_id, 'todo', todo_lici[device.tenant_id]);
  });
  client.on('todo_mod', function(data) {
    ...
    broadcast_tenant(device.tenant_id, 'todo_mod', todo_lici[device.tenant_id][data.id]);
  });
  ...
  client.on('tenant', function(data) {
    broadcast_tenant(device.tenant_id, 'broadcast', id);
  });
